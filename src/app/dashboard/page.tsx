...
const SectionTitle = ({ title, icon: Icon, action }: { title: string; icon?: React.ElementType; action?: React.ReactNode }) => (
  <div className="flex items-center justify-between mb-6">
    <h2 className="text-3xl font-sans font-light tracking-wide text-charcoal-900 dark:text-warm-100 flex items-center gap-2">
...
const EmptyState = ({ icon: Icon, title, desc, action }: { icon: React.ElementType; title: string; desc: string; action?: React.ReactNode }) => (
  <div className="flex flex-col items-center justify-center py-16 text-center">
    ...
    <h3 className="text-2xl font-sans font-light tracking-wide text-charcoal-900 dark:text-warm-100 mb-2">{title}</h3>
    <p className="font-sans font-light tracking-wide text-xl text-charcoal-500 dark:text-charcoal-400 max-w-xs mb-4">{desc}</p>
    {action}
  </div>
);
...
const ArtworkCard = ({ art, onDelete, showDelete }: { art: ArtworkDB; onDelete?: () => void; showDelete?: boolean }) => (
  <div className="group relative rounded-2xl overflow-hidden bg-charcoal-100 dark:bg-charcoal-900 border border-white/50 dark:border-white/10 shadow-sm">
    ...
    <div className="p-4 bg-white/80 dark:bg-charcoal-900/80 backdrop-blur-md absolute bottom-0 left-0 right-0 transform translate-y-full group-hover:translate-y-0 transition-transform duration-300">
      <h3 className="font-sans font-light tracking-wide text-2xl text-charcoal-900 dark:text-warm-100 truncate mb-1">{art.title}</h3>
      <div className="flex items-center gap-3 font-display capitalize text-lg text-charcoal-500 dark:text-charcoal-400">
...
const SavedCard = ({ art, onUnsave }: { art: ArtworkDB; onUnsave: () => void }) => (
  <div className="group relative rounded-2xl overflow-hidden bg-charcoal-100 dark:bg-charcoal-900 border border-white/50 dark:border-white/10 shadow-sm">
    ...
    <div className="p-3 border-t border-white/20 dark:border-white/10">
      <h3 className="font-sans font-light tracking-wide text-2xl text-charcoal-900 dark:text-warm-100 truncate">{art.title}</h3>
      <p className="font-display capitalize text-lg text-charcoal-500 dark:text-charcoal-400 capitalize">{art.artist_name}</p>
    </div>
  </div>
);
      ...
      <main className="flex-1 lg:ml-20 p-4 sm:p-6 lg:p-8 xl:p-10 w-full overflow-x-hidden">
        <div className="max-w-screen-2xl mx-auto space-y-8 pb-20">
          ...
          <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
            <div>
              <h1 className="font-sans font-light text-4xl md:text-5xl text-charcoal-900 dark:text-warm-100 mb-2">
                Welcome back, {userName.split(" ")[0]}
              </h1>
              <p className="font-sans font-light tracking-wide text-xl text-charcoal-600 dark:text-charcoal-300">Here&apos;s a comprehensive overview of your creative journey.</p>
            </div>
              ...
              <motion.div key="overview" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.3 }} className="space-y-8">
                ...
                <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                    ...
                    {[
                      ...
                      <GlassCard key={i} className="p-6 relative group hover:bg-white/90 dark:hover:bg-charcoal-800/90 transition-all duration-300 transform hover:-translate-y-1">
                        <div className="flex items-center gap-3 mb-4">
                          ...
                          <p className="font-sans font-light tracking-wide text-2xl text-charcoal-600 dark:text-charcoal-300">{stat.label}</p>
                        </div>
                  ...
                  <GlassCard className="xl:col-span-1 p-8 bg-gradient-to-br from-charcoal-900 to-charcoal-950 dark:from-[#1A1A1A] dark:to-[#0A0A0A] border-none text-white relative overflow-hidden flex flex-col justify-center items-center text-center">
                    ...
                    <h3 className="font-sans font-light tracking-wide text-2xl text-warm-100 mb-2">Total Comments</h3>
                  ...
                  <GlassCard className="lg:col-span-2 p-6 md:p-8">
                    ...
                    {monthlyData.length > 0 ? (
                      ...
                      <div className="h-[200px] flex flex-col items-center justify-center text-center">
                        ...
                        <p className="font-sans font-light tracking-wide text-xl text-charcoal-500 dark:text-charcoal-400">Upload artworks to see your upload history.</p>
                      </div>
                    )}
                  </GlassCard>
                    ...
                    {realPieData.length > 0 ? (
                      <div className="flex-1 min-h-[200px] relative mt-2">
                        ...
                        <div className="flex flex-wrap justify-center gap-x-3 gap-y-2 mt-2">
                          {realPieData.map((e, i) => (
                            <div key={i} className="flex items-center gap-1.5 font-display capitalize text-lg text-charcoal-600 dark:text-charcoal-300">
                      ...
                      <div className="flex-1 flex flex-col items-center justify-center text-center">
                        ...
                        <p className="font-sans font-light tracking-wide text-xl text-charcoal-500 dark:text-charcoal-400">No categories to display.</p>
                      </div>
                    )}
                  </GlassCard>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  <GlassCard className="p-6 md:p-8">
                    ...
                    <div className="flex flex-col items-center justify-center py-8 text-center">
                      ...
                      <p className="font-sans font-light tracking-wide text-xl text-charcoal-500 dark:text-charcoal-400">No recent activity.</p>
                    </div>
                  </GlassCard>
                  ...
                  <GlassCard className="p-6 md:p-8">
                    ...
                    {topArtwork ? (
                      <div className="mt-4">
                        ...
                        <h3 className="font-sans font-light tracking-wide text-2xl text-charcoal-900 dark:text-warm-100 mb-1">{topArtwork.title}</h3>
                        <p className="font-display capitalize text-xl text-charcoal-500 dark:text-charcoal-400 capitalize mb-4">{topArtwork.category}</p>
                        <div className="grid grid-cols-2 gap-4">
                          <div className="bg-white/50 dark:bg-charcoal-800/50 rounded-xl p-3 border border-white/40 dark:border-white/5">
                            <p className="font-sans font-light tracking-wide text-lg text-charcoal-500 dark:text-charcoal-400 mb-1">Saves</p>
                            <p className="text-xl font-display font-bold text-charcoal-900 dark:text-warm-100">{formatNumber(topArtwork.saves || 0)}</p>
                          </div>
                          <div className="bg-white/50 dark:bg-charcoal-800/50 rounded-xl p-3 border border-white/40 dark:border-white/5">
                            <p className="font-sans font-light tracking-wide text-lg text-charcoal-500 dark:text-charcoal-400 mb-1">Likes</p>
                  ...
                  <GlassCard className="p-6 md:p-8">
                    ...
                    <div className="space-y-4 mt-4">
                      <Link href="/upload" className="w-full flex items-center gap-4 p-4 rounded-2xl bg-gradient-to-r from-accent-terracotta to-[#D4A853] text-white hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1">
                        ...
                        <div className="text-left flex-1"><h4 className="font-display capitalize font-medium text-xl">Upload Artwork</h4><p className="font-display capitalize text-lg text-white/80">Share your latest creation</p></div>
                      ...
                      <button onClick={() => setActiveTab("collections")} className="w-full flex items-center gap-4 p-4 rounded-2xl bg-white/60 dark:bg-charcoal-800/60 hover:bg-white/90 dark:hover:bg-charcoal-700/80 border border-white/40 dark:border-white/10 transition-all duration-300 transform hover:-translate-y-1 text-charcoal-900 dark:text-warm-100">
                        ...
                        <div className="text-left flex-1"><h4 className="font-display capitalize font-medium text-xl text-charcoal-900 dark:text-warm-100">My Collections</h4><p className="font-display capitalize text-lg text-charcoal-500 dark:text-charcoal-400">{collections.length} collections</p></div>
                      ...
                      <button onClick={() => { setActiveTab("settings"); setActiveSettingsTab("profile"); }} className="w-full flex items-center gap-4 p-4 rounded-2xl bg-white/60 dark:bg-charcoal-800/60 hover:bg-white/90 dark:hover:bg-charcoal-700/80 border border-white/40 dark:border-white/10 transition-all duration-300 transform hover:-translate-y-1 text-charcoal-900 dark:text-warm-100">
                        ...
                        <div className="text-left flex-1"><h4 className="font-display capitalize font-medium text-xl text-charcoal-900 dark:text-warm-100">Edit Profile</h4><p className="font-display capitalize text-lg text-charcoal-500 dark:text-charcoal-400">Update bio and links</p></div>
                        <ChevronRight className="w-5 h-5 text-charcoal-400" />
                      </button>
                    </div>
                  </GlassCard>
                ...
                <GlassCard className="p-6 md:p-8">
                  ...
                  {showCreateCollection && (
                    <div className="mb-6 p-5 rounded-2xl bg-white/50 dark:bg-charcoal-800/50 border border-white/40 dark:border-white/10">
                      <h3 className="font-sans font-light tracking-wide text-2xl text-charcoal-900 dark:text-warm-100 mb-4">Create New Collection</h3>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                      {collections.map(col => (
                        <Link href={`/collection/${col.id}`} key={col.id} className="group relative rounded-2xl border border-white/40 dark:border-white/10 bg-white/50 dark:bg-charcoal-800/50 p-5 hover:bg-white/80 dark:hover:bg-charcoal-700/60 transition-all duration-300 block">
                          ...
                          <h3 className="font-sans font-light tracking-wide text-2xl text-charcoal-900 dark:text-warm-100 mb-1 truncate">{col.name}</h3>
                          {col.description && <p className="font-display capitalize text-lg text-charcoal-500 dark:text-charcoal-400 mb-3 line-clamp-2">{col.description}</p>}
                          <div className="flex items-center justify-between">
                            <span className="font-display capitalize text-lg text-charcoal-500 dark:text-charcoal-400">{col.artwork_count ?? 0} artworks</span>
                  ...
                  {[
                    ...
                    <GlassCard key={i} className="p-5">
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <p className="font-sans font-light tracking-wide text-xl text-charcoal-600 dark:text-charcoal-300">{s.label}</p>
                      </div>
                        ...
                        {realPieData.map((e, i) => (
                          <div key={i} className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="font-sans font-light tracking-wide text-xl text-charcoal-900 dark:text-warm-100">{e.name}</span>
                            </div>
                            <span className="font-display capitalize text-xl text-charcoal-600 dark:text-charcoal-300">{e.value} artworks</span>
                          </div>
                        ))}
                      </div>
                    </div>
              ...
              <motion.div key="community" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.3 }} className="space-y-6">
                <GlassCard className="p-6 md:p-8">
                  <SectionTitle title="Discover Artists" icon={Users}
                    action={<span className="font-display capitalize text-xl text-charcoal-500 dark:text-charcoal-400">{artists.length} artists</span>}
                  />
                  {artists.length > 0 ? (
                      ...
                      {artists.map(artist => (
                        <div key={artist.id} className="flex items-center gap-4 p-4 rounded-2xl bg-white/50 dark:bg-charcoal-800/50 border border-white/40 dark:border-white/10 hover:bg-white/80 dark:hover:bg-charcoal-700/60 transition-all duration-300">
                          ...
                          <div className="flex-1 min-w-0">
                            <h3 className="font-sans font-light tracking-wide text-2xl text-charcoal-900 dark:text-warm-100 truncate">{artist.display_name}</h3>
                            <p className="font-display capitalize text-lg text-charcoal-500 dark:text-charcoal-400 truncate">{artist.bio || "Artist"}</p>
                          </div>
                          <button
                            onClick={() => artist.is_following ? handleUnfollow(artist.id) : handleFollow(artist.id)}
                            className={cn("flex-shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-xl font-sans font-light tracking-wide text-sm font-semibold transition-all duration-200",
                              artist.is_following
